...
次に示すコードは、executeQuery()を用いてselect文を実行している(line 31)。Connectionインスタンスの生成(line 28)、Statementインスタンスの生成は(line 30)、先に紹介したexecuteUpdate()を用いるサンプル・コードと変わらないことを確認してほしい(line 13, 15)。
...
try-catch文が入れ子になっているのは、rollback()がSQLExceptionをスローする可能性があるからである。しかし、トランザクション処理が終われば、自動コミットをオンに戻すことが広く推奨されている。自動コミットをオフにした後(line 16)、内側のtryブロックで例外がスローされても、finally句の中の自動コミットをオンに戻すコードは必ず実行される(line 76)。

外側のtry-with-resources文は、データベース接続をクローズするためのものである。try-with-resources文のfinally句に自動コミットをオンに戻すコードを置くことはできない。finally句に到達した時には既に接続がクローズされているので、setAutoCommit()がSQLExceptionをスローするからである。
