...
データベースへの接続が成功したら、データベースに対してSQL文を実行できる。しかし、SQL文を実行するには、Statementインスンタスが必要不可欠である。次のコードのように、Statementインスタンスは、Connectionから作成する(line 15)。Connectionと同様、不要になったStatementはクローズしなければならない。したがって、Statementをtry-with-resources文の資源として宣言することが少なくない。後述するPreparedStatementも同様である。

**補足:**
このコードではStatementを明示的にクローズしていない。Connectionがクローズされたとき、その接続から作成されたStatementも同時にクローズされるからである。
